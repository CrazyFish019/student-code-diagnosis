...
安装版启动后会在 Windows 系统托盘保留“学生代码诊断助手”图标。关闭浏览器页面
不会直接结束后台服务；可右键托盘图标选择“打开诊断页面”或“退出程序”，也可使用
页面侧边栏底部的“退出诊断工具”按钮。AI 正在诊断时页面退出按钮会暂时禁用，避免
中断请求。重复启动程序只会重新打开现有页面，不会创建第二个后台服务。
...
- 输入、输出和提示；
- 公开样例输入与输出；
- 待诊断的 C++ 或 Python 3 源代码（手动粘贴模式当前以 C++ 为主）。

手动粘贴模式不会编译或运行代码，也没有隐藏测试数据。提交详情模式会使用 OJ
已经产生的判题状态统计。测试点详情和学生源码默认折叠并只在本机显示；只有教师
选中测试点并主动开启“发送选中的测试点详情给模型”后，才会把相应输入、标准输出、
时间、内存及运行错误信息作为诊断证据。开启该选项后，程序会在独立临时目录中执行：
对于 C++，编译一次学生源码；对于 Python 3，使用安装包内置的独立 Python 执行器。
两种语言都只运行教师选中的测试点，并把本地捕获的实际输出一并发送给模型。
...
提交接口不提供可靠的学生程序
输出，因此 WA 详情不会伪造或展示“学生输出”。未主动选择测试点时，本工具不会重新
运行代码。结果使用
...
- 测试点右上角复选框独立控制是否选中，点击测试点方块只负责打开详情；
...
- 本地只运行教师勾选的测试点，复用受控Runner的时间限制和进程树清理，任务结束后
  删除源码、可执行文件和运行目录；C++与Python分别使用独立临时目录，不共享编译或
  运行状态；该机制不是OS安全沙箱，不能抵御蓄意恶意代码；
